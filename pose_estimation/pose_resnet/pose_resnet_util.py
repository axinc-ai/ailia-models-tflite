...
def compute(interpreter_pose, original_img, offset_x, offset_y, scale_x, scale_y, dtype):

    input_details = interpreter_pose.get_input_details()
    output_details = interpreter_pose.get_output_details()
    ...
    angle_y = 0
    angle_z = 0
    for j in range(const.POSE_KEYPOINT_CNT):
        i = ailia_to_mpi[j]
        z = 0
        interpolated = 0
        if j == const.POSE_KEYPOINT_BODY_CENTER:
            ...
            score = min(min(min(
                ...
                maxvals[0, ailia_to_mpi[const.POSE_KEYPOINT_HIP_LEFT], 0]),
